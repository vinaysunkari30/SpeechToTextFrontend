...
const Home = () => {
  ...
  const getTranscriptionsList = async() =>{
    try{
      ...
      const response = await axios('https://speechtotextbackend-2owz.onrender.com/transcriptions', {headers:{
  ...
  const onConvertAudio = async () => {
    ...
    try {
      if (file) {
        ...
        const recordedFile = new File([blob], 'recorded-audio.mp3', { type: blob.type });
      ...
      setLoading(true)
      const { data } = await axios.post('https://speechtotextbackend-2owz.onrender.com/upload-audio', formData, {
        headers: {
      ...
      {fileName && (
        <div className="text-purple-950 text-md font-medium mt-2 mb-3 flex justify-center items-center gap-3">
          ...
          <TailSpin
            ...
            visible={isLoading}
          />: <button className="convert ml-3" onClick={onConvertAudio}>Convert</button>}
        </div>
      )}
    </>
  )
  ...
  const renderRecordUptoMedium = () =>(
    <div className="flex flex-col justify-center items-center mb-3 mt-3">
      <div className='flex flex-col justify-center items-center'>
        {startClicked ? '': <div className='p-1 w-100'><p className='text-purple-950 mb-5 text-2xl font-semibold font-serif text-center'>Start Recording and See your words as a Text</p></div>}
        <div className='flex gap-5 justify-center mb-4'>
          <button className='start-btn text-white' onClick={startRecording}>Start</button>
          <button className='stop-btn text-white' onClick={stopRecording}>Stop</button>
        </div>
        {mediaBlobUrl && <audio src={mediaBlobUrl} controls className='mb-4'/>}
      </div>
      { mediaBlobUrl && <button className="convert ml-3 mb-3" onClick={onConvertAudio}>Convert</button>}
      {isLoading? 
        <TailSpin
            height="40"
            width="40"
  ...
  return(
    <div className='home-div flex flex-col justify-start items-center pb-3'>
      <div className='w-full flex justify-between items-center bg-cyan-800 p-2 mb-3'>
        <p className='logo text-white text-2xl font-serif font-semibold'>Speech to Text</p>
        <button className='logout' onClick={onLogout}>Logout</button>
      </div>
      <h1 className='text-center text-2xl text-purple-950 font-semibold font-serif underline decoration-2 mb-2'>Speech to Text</h1>
      <p className='text-slate-600 text-center font-medium text-xl mb-4'>Turn your voice into meaningful words<br/> with Artificial Intelligence</p>
      <div className='tabs flex gap-3 mb-2 lg:hidden mt-3 w-100 items-center'>
        <div className='flex flex-1'>
          <button onClick={handleUploadTab} className={`tab-btn text-sky-900 w-full text-3xl font-extrabold border-2
          ${activeTab === 'Upload' ? 'underline decoration-2 bg-sky-900': ''}`}>Upload</button>
        </div>
        <div className='flex flex-1'>
            <button onClick={handleRecordTab} className={`tab-btn text-sky-900 w-full text-3xl font-extrabold
          ${activeTab === 'Record' ? 'underline decoration-2 bg-sky-900': ''}`}>Record</button>
        </div>
        <div className='flex flex-1'>
          <button onClick={handleTextTab} className={`tab-btn text-sky-900 text-3xl font-extrabold
          ${activeTab === 'Transcriptions' ? 'underline decoration-2 bg-sky-900': ''}`}>Transcriptions</button>
        </div>
      </div> 
      {activeTab==='Transcriptions' &&
        <div className='pl-2 pr-2 lg:hidden'>
          <ul className='mt-2 p-4 pt-2 rounded-lg trans-div'>
            <p className='text-4xl mb-3 font-medium text-center underline decoration-2 font-serif text-purple-950'>Transcriptions</p>
        ...
        <div className='flex-1 justify-center flex items-center'>
          {activeTab==='Upload' || activeTab==='Record' ?
            <div className='text-left lg-hidden'>
              <h2 className='text-sky-900 font-bold text-2xl '>Transcription:</h2>
              <div className='border-2 border-sky-900 rounded-xl w-100 md:w-90 h-50 break-words overflow-y-scroll no-scrollbar'>
                <p className='p-2 text-md font-semibold text-purple-950'>{transcription}</p>
              </div>
            </div> : ''
          }
        </div>
      ...
      <div className='hidden lg:flex flex flex-row'>
        <div className='flex flex-row text-center flex-1 items-center'>
          <div className='flex flex-col flex-1 justify-center items-center gap-3 w-100 pt-5'>
            <div className='flex flex-row gap-10'>
              <div className='flex flex-1'>
                <button onClick={handleUploadTab} className={`tab-btn text-sky-900 w-full text-3xl font-extrabold
                ${activeTab === 'Upload' ? 'underline decoration-2 bg-sky-900': ''}`}>Upload</button>
              </div>
              <div className='flex flex-1'>
                <button onClick={handleRecordTab} className={`tab-btn text-sky-900 w-full text-3xl font-extrabold
          ...
          <div className='flex-1 trans-div bg-white rounded-xl pl-8 pr-8 pt-1 max-h-130 overflow-y-auto no-scrollbar pb-5'>
            <p className='text-4xl mt-4 mb-5 font-semibold text-center font-serif underline decoration-2 text-purple-950'>Transcriptions</p>
            <ul className='mt-1'>
              {transcriptionsList.map(each => (
                <Transcriptions key={each.id} data={each}/>
              ))
